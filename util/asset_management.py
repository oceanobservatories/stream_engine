...
class Deployment(object):
    ...
    def get_cal_values(self):
        ...
        for each in self._get_calibration():
            ...
            for each in data:
                val, card, dims, cstart, cstop = self._extract_from_cal(each)
                try:
                    cstart, cstop = self._valid_times(dstart, dstop, cstart, cstop)
                    val = self._format(val, card, dims)
                    d.setdefault(name, []).append((val, cstart, cstop))
                except TimeBoundsException:
                    continue
        return d

    def _format(self, val, cardinality, dimensions):
        if dimensions == [1] and cardinality == 0:
            return val[0]
        else:
            return val
    ...
    def _extract_from_cal(self, data):
        ...
        return val, cardinality, dimensions, start, stop
