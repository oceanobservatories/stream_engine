...
def is_equal(chunk1, chunk2):
    """
    Tests whether two chunks are logically equal.

    Note: this will always return false if the chunks contain NaN and are not the
    same reference.
    ...
    for key in chunk1:
        ...
        if chunk1[key]['data'] is not chunk2[key]['data']:
            if not numpy.array_equal(chunk1[key]['data'], chunk2[key]['data']):
                return False
    ...
    for key in chunk:
        if key != 7:
            try:
                ...
                # binned_statistic uses the last time inclusively.
                b = times + [max(chunk[7]['data'][-1], times[-1])]
