...
class CSVGenerator(object):
    ...
    def create_async_csv(self, path):
        ...
        for stream_key, deployment, ds in self.stream_data.groups():
            file_path = '%s/deployment%04d_%s.csv' % (base_path, deployment, stream_key.as_dashed_refdes())
    ...
    def write_to_zipfile(self, zf):
        for stream_key, deployment, ds in self.stream_data.groups():
            with tempfile.NamedTemporaryFile() as tf:
                ...
                zf.write(tf.name, 'deployment%04d_%s.csv' % (deployment, stream_key.as_dashed_refdes(),))
    ...
    def _write_csv_out(self, fileout, stream_key, deployment, ds):
        ...
        writer = csv.DictWriter(fileout, to_use)
        writer.writeheader()
