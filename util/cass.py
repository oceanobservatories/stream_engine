...
class JoinedFuture(ResponseFuture):
    ...
    def __init__(self, futures):
        ...
        self._joined_final_result = None
    ...
    def result(self):
        if self._joined_final_result is None:
            self._joined_final_result = []
            try:
                while True:
                    self._joined_final_result.extend(self.current_future.result())
                    self.current_future = next(self.futures)
            except StopIteration:
                pass
        return self._joined_final_result
