...
def insert_dataset(stream_key, dataset, session=None, prepared=None, query_consistency=None ):
    ...
    data_lists['subsite'] = [stream_key.subsite] * size
    data_lists['bin'] = [data_bin] * size
    data_lists['node'] =[stream_key.node] * size
    data_lists['sensor'] = [stream_key.sensor] * size
    data_lists['method'] = [stream_key.method] * size
    # id and provenance are expected to be UUIDs so convert them to uuids
    data_lists['id'] = [uuid.UUID(x) for x in  dataset['id'].values.tolist()]
    data_lists['provenance'] = [uuid.UUID(x) for x in dataset['provenance'].values.tolist()]
    for i in arrays:
    ...
    for dc in cols:
        ...
        if dc in dataset and dc not in data_lists:
            if '_FillValue' in dataset[dc].attrs:
                ...
                data_lists[dc] = temp_val.tolist()
            else:
                data_lists[dc] = dataset[dc].values.tolist()
    ...
    if bin_meta is None or engine.app.config['SAN_CASS_OVERWRITE']:
        ...
        to_insert = []
        for i in range(size):
            row = [data_lists[col][i] for col in cols]
