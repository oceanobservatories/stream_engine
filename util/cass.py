...
def fetch_all_data(stream_key, time_range, session=None, prepared=None, query_consistency=None):
    ...
    bins = [(x,) for x in xrange(time_to_bin(start), time_to_bin(stop), 1)]

    futures = []
    futures.append(execution_pool.apply_async(execute_unlimited_query, (stream_key, cols, bins)))
...
@cassandra_session
@log_timing
def execute_unlimited_query(stream_key, cols, time_bins, session=None, prepared=None,
                  query_consistency=None):
    ...
    my_query = prepared[query_name]

    result = list(execute_concurrent_with_args(session, my_query, time_bins, concurrency=50))
    return result
