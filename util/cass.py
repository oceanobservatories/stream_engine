...
def fetch_data_sync(stream_key, time_range, session=None, prepared=None):
    engine.app.logger.info("Starting synchronous query")
...
#def fetch_nth_data(stream_key, time_range, num_points=50, chunk_size=100, session=None, prepared=None):
    #"""
    #Given a time range, generate evenly spaced times over the specified interval

    #This is a very approximate query. It's best used with a time range that precisely spans the data.
    #If the time range is much larger than the time range the data exists on, the query may only return
    #one result.
    ...
    #engine.app.logger.info("Starting limit based query")
    ...
    ##first = session.execute(base % 'time' + ' and time>=%s order by method asc, time asc limit 1', (start,))
    ##last = session.execute(base % 'time' + ' and time<=%s order by method desc, time desc limit 1', (stop,))
    ...
    #rows = execute_concurrent_with_args(session, query, times, concurrency=50)

    #rows = (row[1][0] for row in rows if row[0] and row[1] and len(row[1]) > 0)

    ## deduplicate and sort
    ##TODO: look into using numpy.unique for this
    #uniq = {}
    #for row in rows:
...
def fetch_nth_data(stream_key, time_range, num_points=50, chunk_size=100, session=None, prepared=None):
    ...
    # deduplicate and sort
    #TODO: switch this to numpy.unique
    uniq = {}
    for row in rows:
        key = "{}{}".format(row[0], row[-1]) # this should include more than the time and the last value
        uniq[key] = row
