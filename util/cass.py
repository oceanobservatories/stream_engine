...
def fetch_l0_provenance(stream_key, provenance_values, deployment, session=None, prepared=None,
    ...
    if len(provenance_arguments) != len(ret_val):
        log.warn("Could not find %d provenance entries", len(provenance_arguments) - len(ret_val))
    prov_dict = {str(row.id) : {'file_name' : row.file_name, 'parser_name' : row.parser_name, 'parser_version' : row.parser_version} for row in ret_val}
    return prov_dict
