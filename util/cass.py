...
def fetch_all_data(stream_key, time_range, strict_range=False, chunk_size=100, session=None,
    ...
    for i in xrange(0, num_points, chunk_size):
        futures.append(execution_pool.apply_async(execute_unlimited_query, (stream_key, cols, times[i:i + chunk_size], time_range, strict_range)))
...
@cassandra_session
@log_timing
def execute_unlimited_query(stream_key, cols, times, time_range, strict_range=False, session=None, prepared=None,
                  query_consistency=None):
    if strict_range:
        query = session.prepare("select %s from %s where subsite='%s' and node='%s'"
                                " and sensor='%s' and bin=? and method='%s' and time>=%s and time<=?"
