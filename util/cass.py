...
class ConcurrentBatchFuture(ResponseFuture):
    ...
    def __init__(self, stream_key, cols, times):
        ...
        self.times = times
        self.q_per_page = 0
        """Number of queries to run per page"""
        self.q_per_proc = 10
        """Number of queries to assign per process"""
        self.c_per_proc = 50
        """Number of queries to run concurrently per process"""
        self.blocking_init = True
        """Whether or not to block and fetch results immediately on construction"""
