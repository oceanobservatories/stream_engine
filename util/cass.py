...
def fetch_data_sync(stream_key, time_range, strict_range=False, session=None, prepared=None, query_consistency=None):
    ...
    if not strict_range:
        ...
        first = session.execute(first_statement)
    ...
    times = [(b, start, stop) for b in
             xrange(time_to_bin(start, stream_key.stream.name), time_to_bin(stop, stream_key.stream.name) + 1)]
