...
def fetch_nth_data(stream_key, time_range, strict_range=False, num_points=1000, chunk_size=100, session=None,
    ...
    start = time_range.start
    stop = time_range.stop
    metadata_bins, bin_metadata, total_data = get_cass_bin_information(stream_key, time_range)
    results = []
    ...
    # We have a lot of bins so just grab the first from each of the bins
        ...
        bin_information = [(x, bin_metadata[x][1], bin_metadata[x][2]) for x in metadata_bins]
        bin_information = sorted(bin_information, key=lambda info: info[0])
        bin_queue = deque(bin_information)
        # Create our time array of sampling times
        times = numpy.linspace(start, stop, num_points)
