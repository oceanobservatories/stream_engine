...
from util.metadata_service import (CASS_LOCATION_NAME, get_cass_location_metadata, get_location_metadata,
                                   get_partition_metadata_record, create_partition_metadata_record,
                                   update_partition_metadata_record)
...
def _get_stream_row_count(stream_key, data_bin):
    ...
    return SessionManager.execute(count_query, (stream_key.subsite, stream_key.node, stream_key.sensor, data_bin, stream_key.method))[0][0]
...
def insert_dataset(stream_key, dataset):
    ...
    bin_meta = get_partition_metadata_record(stream_key, data_bin, CASS_LOCATION_NAME)
    ...
    if bin_meta is None:
        ...
        create_partition_metadata_record(stream_key, data_bin, CASS_LOCATION_NAME, first, last, count)
        ret_val = 'Inserted {:d} particles into Cassandra bin {:d} for {:s}.'.format(count, data_bin, stream_key.as_refdes())
    else:
        ...
        update_partition_metadata_record(bin_meta)
