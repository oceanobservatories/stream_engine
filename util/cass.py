...
def fetch_annotations(stream_key, time_range, session=None, prepared=None, query_consistency=None, with_mooring=True):
    ...
    # This is necessary because of the way the Cassandra primary key
    # is setup for the annoations table.
    ...
    # query where annotation effectivity is whithin the query time range
    # or stadles the end points
    for success, rows in execute_concurrent_with_args(session, query1, args, concurrency=50):
        if success:
            result.extend(list(rows))

    temp = []
    for success, rows in execute_concurrent_with_args(session, query2, args, concurrency=50):
        if success:
