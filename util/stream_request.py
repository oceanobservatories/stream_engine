...
from preload_database.model.preload import Parameter, Stream, UnfulfilledParameterException
...
from util.datamodel import compile_datasets, create_empty_dataset
...
class StreamRequest(object):
    ...
    def _execute_algorithm(self, parameter, kwargs):
        ...
        log.debug('<%s> _execute_algorithm Keyword Args %r', self.request_id, {k: len(kwargs[k]) for k in kwargs})
            ...
            if source == 'CAL':
                ...
                if cal is not None:
                    ...
                    if np.any(np.isnan(cal)):
                        msg = '<{:s}> There was not coefficient data for {:s} all times in deployment ' \
    ...
    def _create_derived_product(self, dataset, stream_key, param, deployment, source_dataset=None):
        ...
        try:
            ...
            if set(kwargs) == set(function_map):
                ...
                missing = set(function_map).difference(kwargs)
                error_info = {'derived_id': param.id, 'derived_name': param.name,
                              'derived_display_name': param.display_name, 'missing': []}
                for key in missing:
                    source, value = function_map[key]
                    missing = {
                        'source': source,
                        'value': value
                    }
                    error_info['missing'].append(missing)
        ...
        except UnfulfilledParameterException:
            log.error('<%s> Unable to create derived product: %r error locating data', self.request_id, param)
    ...
    def _import_interpolated(self, stream_key, param):
        ...
        except UnfulfilledParameterException as e:
            log.error('<%s> Unable to interpolate data: %r from: %r error locating data: %r',
                      self.request_id, param, stream_key.as_refdes(), e.message)
    ...
    def calculate_derived_products(self):
        ...
        for sk in self.datasets:
            if sk.is_virtual:
                log.debug('<%s> Compute virtual stream', self.request_id)
                source_key = None
