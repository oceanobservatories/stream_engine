...
def concatenate_and_write(datasets, job_dir, group_name):
    ds = compile_datasets(datasets)
    add_dynamic_attributes(ds)
    write_netcdf(ds, os.path.join(job_dir, get_name(ds, group_name)))


def aggregate_netcdf_group(job_dir, files, group_name):
    datasets = []
    MAX_SIZE = 500e6
    accum_size = 0
    ...
    for f in sorted(files):
        ...
        if accum_size > MAX_SIZE:
            concatenate_and_write(datasets, job_dir, group_name)
            accum_size = size
            datasets = []
...
def aggregate_netcdf(job_dir):
    ...
    for group in groups:
        aggregate_netcdf_group(job_dir, groups[group], group)
