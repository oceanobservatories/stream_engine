import engine
import logging
from api import get_partition_metadata_records
...
_Data = namedtuple('Data', ['bin', 'store', 'count', 'first', 'last'])


def _get_first_possible_bin(t):
    t -= engine.app.config['MAX_BIN_SIZE_MIN'] * 60
    return long(t)


def _time_in_bin_units(t):
    return long(t)
...
def _query_partition_metadata_before(stream_key, time_start):
    ...
    """
    start_bin = _time_in_bin_units(time_start)
    partitionMetadataRecordList = get_partition_metadata_records(stream_key)
    partitionMetadataRecordList.sort(key=itemgetter('bin'), reverse=True)
    result = []
    for rec in partitionMetadataRecordList:
        if rec['bin'] <= start_bin:
            result.append(_Data(rec['bin'], rec['store'], rec['count'], rec['first'], rec['last']))
        if len(result) == 4:
            break
    return result
...
def _query_partition_metadata(stream_key, time_range):
    ...
    """
    start_bin = _get_first_possible_bin(time_range.start)
    end_bin = _time_in_bin_units(time_range.stop)
    partitionMetadataRecordList = get_partition_metadata_records(stream_key)
    result = []
    for rec in partitionMetadataRecordList:
        ...
        for key in san_bins.keys():
            if key in cass_bins:
                ...
                if cass_count != san_count:
                    _log.warn("Metadata count does not match for bin %d - SAN: %d  CASS: %d", key, san_count, cass_count)
                    messages.append(
