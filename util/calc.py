...
class NetCDF_Generator(object):
    ...
    def write_to_zipfile(self, r, zf):
        ...
        for source in datasets:
            datasets[source] = self.group_by_source(source, datasets[source], chunk_valid, chunk_times[chunk_valid], primary_key)
    ...
    def group_by_source(self, source_key, source_group, chunk_valid, time_data, primary_key):
        this_group = xray.Dataset()
        for param_id in self.pd_data:
            if param_id == self.tp or self.pd_data[param_id][primary_key.as_refdes()]['source'] != source_key:
                continue
