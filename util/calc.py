...
def get_particles(streams, start, stop, coefficients, qc_parameters, limit=None, custom_times=None, custom_type=None):
    ...
    #for index in range(len(pd_data[time_param][key.as_refdes()]['data'])):
        ...
        for param in primary_key.stream.parameters:
            if param.id in pd_data:
                ...
                try:
                    ...
                    #print "Exception: ", e
                    #print "Issue getting data for {}, has keys: {}, looking for {}".format(param.id, pd_data[param.id].keys(), primary_key.as_refdes())
                    #print ""
                    engine.app.logger.info("Failed to get data for {}: ".format(param.id, e))
                    continue
            ...
            if qc_parameters.get(param.name) is not None:
                for qc_function_name in qc_parameters.get(param.name):
                    ...
                    if qc_function_results in pd_data:
                        value = arb(pd_data[qc_function_results])['data'][index]
...
def fetch_pd_data(stream_request, streams, start, stop, coefficients, limit):
    ...
    engine.app.logger.info("Pulling data from {} stream(s): {}".format(len(stream_request.stream_keys), [x.as_refdes() for x in stream_request.stream_keys]))

    pd_data = {}
    first_time = None
    for key_index, key in enumerate(stream_request.stream_keys):
        if key.stream.is_virtual:
            engine.app.logger.info("Skipping virtual stream: {}".format(key.stream.name))
            continue
        ...
        if len(cass_data) == 0:
            engine.app.logger.warning("Query for {} returned no data".format(key.as_refdes()))
            continue
        ...
        for param in parameters:
            ...
            #if key_index != 0 and not primary_stream.is_virtual:
            #if key_index != 0:
                ## perform interpolation
                #engine.app.logger.info("Trying to interpolate {} {} (PD{}) data to {} points".format(key.as_refdes(), param.name, param.id, len(first_time)))
                #data_slice = interpolate_list(first_time, df['time'].values, data_slice)
    ...
    if time_param.parameter_type ==  FUNCTION:
        ...
        if time_param.id not in pd_data:
            raise MissingTimeException("Time param is missing")
    ...
    for param in primary_key.stream.parameters:
        if param.id not in pd_data:
            ...
            if stream_request.qc_parameters.get(param.name) is not None:
                _qc_check(param, pd_data, stream_request)

    return pd_data


def _qc_check(parameter, chunk, stream_request):
...
def calculate_derived_product(param, coeffs, pd_data, primary_key, level=1):
    ...
    """
    engine.app.logger.info("\n"+((level-1)*4-1)*' ')
    spaces = level*4*' '
    engine.app.logger.info("{}Running dpa for {} (PD{}){{".format((level-1)*4*' ', param.name, param.id))
    ...
    try:
        ...
        engine.app.logger.info("{}aborting - {}".format(spaces, e.message))
    else:
        if param.id not in pd_data:
            pd_data[param.id] = {}
        ...
        engine.app.logger.info(spaces+"returning data")
    engine.app.logger.info("{}}}".format((level-1)*4*' '))
...
def build_func_map(parameter, coefficients, pd_data, primary_key):
    ...
    #main_times = pd_data[primary_key.stream.time_parameter][main_stream_refdes]['data']
    main_times = arb(pd_data[7])['data']
    ...
    for key in func_map:
        if PDRef.is_pdref(func_map[key]):
            ...
            if pdRef.chunk_key not in pd_data:
                raise StreamEngineException('Required parameter %s not found in chunk when calculating %s (PD%s) ' % (func_map[key], parameter.name, parameter.id))
...
def create_netcdf(r, pd_data):
    ...
    with tempfile.NamedTemporaryFile() as tf:
        with netCDF4.Dataset(tf.name, 'w', format='NETCDF4') as ncfile:
            ...
            chunk_times = arb(pd_data[r.stream_keys[0].stream.time_parameter])['data']
            ...
            for param_id in pd_data:
                ...
                data = arb(pd_data[param_id])['data']
                source = arb(pd_data[param_id])['source']
                if param_id == 7:
                    group = ncfile
