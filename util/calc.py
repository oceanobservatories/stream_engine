...
def get_particles(streams, start, stop, coefficients, limit=None, custom_times=None, custom_type=None):
    ...
    pd_data = fetch_pd_data(stream_request, streams, start, stop, coefficients, limit, custom_times)

    ## convert data into a list of particles
    particles = []
    for key in stream_keys:
        length = len(pd_data.itervalues().next()['data'])
        for index in range(length):
            ...
            particle['pk']['deployment'] = pd_data['deployment']['data'][index]
            particle['provenance'] = str(pd_data['provenance']['data'][index])

            for param in key.stream.parameters:
                if param.id in pd_data:
                    val = pd_data[param.id]['data'][index]
...
def fetch_pd_data(stream_request, streams, start, stop, coefficients, limit, custom_times):
    """
    ...
    for param in key.stream.parameters:
        if param.id not in pd_data:
            if param.parameter_type == FUNCTION:
                ...
                calculate_derived_product(param, stream_request.coefficients, pd_data)
            else:
                engine.app.logger.warning("Required parameter not present: {}".format(param.name))
...
def calculate_derived_product(param, coeffs, pd_data, level=1):
    """
    ...
    spaces = level*4*' '
    engine.app.logger.info("{}Running dpa for {}{{".format((level-1)*4*' ', param.name))
...
def build_func_map(parameter, pd_data, coefficients, spaces=""):
    ...
    if 7 not in pd_data:
        engine.app.logger.error("Time not in pd_data")
        return args

    times = pd_data[7]['data']
    for key in func_map:
...
class StreamRequest(object):
    ...
    def _initialize(self, needs_only):
        ...
        if len(self.needs_params) > 0:
            engine.app.logger.warning("Couldn't find all required params, still need: {}".format(self.needs_params))
