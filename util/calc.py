...
from util.cass import get_streams, get_distinct_sensors, fetch_nth_data, fetch_all_data,fetch_cassandra_bin, \
...
def SAN_netcdf(streams, bins):
    ...
    for data_bin in bins:
        ...
        cols, data = fetch_cassandra_bin(stream, data_bin)
        ...
        for deployment, deployment_df in df.groupby('deployment'):
            ...
            log.warn('Offloading {:s} deployment {:d} to {:s} - There are  {:d} particles'.format(str(stream),
                                                                        deployment, nc_file_name, len(deployment_df)))
            # # create netCDF file
            with open(nc_file_name, 'w') as tf:
                with netCDF4.Dataset(tf.name, 'w', format='NETCDF4') as ncfile:
                    ...
                    for param in parameters:
                        if param.name != 'time':
                            ...
                            data_slice = deployment_df[param.name].values
                            if param.is_array:
                                #unpack
                            ...
                            data = deployment_df.index.values
                        dims = [time_dim]
                        if len(data.shape) > 1:
                        ...
                        var[:] = data
                    #add dimensions for this that we do not have in the normal string parameters but that are stored
                    # in cassandra
                    for missing in list(set(deployment_df.columns) - visited):
                        data = deployment_df[missing].values
...
def get_nc_filename(stream, nc_directory, deployment):
    ...
    """
    directory = nc_directory + 'deployment_{:d}/'.format(deployment)
    if not os.path.exists(directory):
        os.makedirs(directory)
    base = directory + stream.stream_name + "{:s}.nc"
    if not os.path.exists(base.format("")):
        return base.format("")
    else:
        index = 1
        while os.path.exists(base.format('_' + str(index))):
            index += 1
        return base.format('_' + str(index))
    ...
    for time_bin, num_data_points in to_sample:
        ...
        if os.path.exists(direct):
            ...
            for deployment in deployments:
                full_path = direct + deployment
                if os.path.isdir(full_path):
                    new_data = get_deployment_data(full_path, stream_key.stream_name, num_data_points, time_range)
                    count = 0
                    for k,d in new_data.iteritems():
                        #keep track of missed data here
                        if count == 0:
                            count = len(d)
                        if k in data:
                            data[k] = numpy.append(data[k], d)
                        else:
                            data[k] = d
                    ...
                    for var in dataset.variables:
                        ...
                        # As a workaround need to select all to get an numpy array and than take selection from that.
                            ...
                            if var in data:
                                ...
                                data[var] = temp
                    # we got data from a file.  Exit now
                    break
    return data
