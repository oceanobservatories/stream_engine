...
def do_qc_stuff(primary_key, stream_data, parameters, qc_stream_parameters):
    ...
    for deployment in stream_data.deployments:
        ...
        for param in parameters:
            ...
            qc_results_values = numpy.zeros_like(time_data).astype(numpy.int8, copy=False)
            qc_ran_values = numpy.zeros_like(time_data).astype(numpy.int8, copy=False)
            ...
            for qc_function_name in qc_stream_parameters.get(param.name, []):
                ...
                if qc_function_results in pd_data\
                    ...
                    qc_results_mask = numpy.full_like(time_data, int(qc_cached_function.qc_flag, 2)).astype(numpy.int8, copy=False)
