...
def get_pickle(streams, start, stop, coefficients, qc_parameters, limit=None,
                  include_provenance=False, include_annotations=False, strict_range=False, location_information={}, request_uuid='', disk_path=None):
    """
    ...
    for k, d, ds in stream_data.groups():
        if k not in stream_keys:
            stream_keys[k] = {}
        deployments = stream_keys[k]
        deployments[d] = ds
    if disk_path is None:
        return pickle.dumps(stream_keys)
    else:
        file_paths = []
        ...
        if not os.path.isdir(base_path):
            ...
            with open(file_path, 'wb') as pick_file:
                pickle.dump(stream_keys, pick_file)
