...
def fetch_stream_data(stream_request, streams, start, stop, coefficients, limit, provenance_metadata, annotation_store):
    ...
    for dep_num in primary_deployments:
        ...
        if lon_sk.stream.lat_param_id is not None:
            interped_data = interpolate_list(primary_stream_len,
                                            pd_data[lon_sk.stream.time_parameter][lon_sk.as_refdes()]['data'],
                                            pd_data[lon_sk.stream.lon_param_id][lon_sk.as_refdes()]['data'])
        ...
        depth_sk = stream_request.lon_stream_key
        if depth_sk.stream.lat_param_id is not None:
            interped_data = interpolate_list(primary_stream_len,
                                            pd_data[depth_sk.stream.time_parameter][depth_sk.as_refdes()]['data'],
                                            pd_data[depth_sk.stream.depth_param_id][depth_sk.as_refdes()]['data'])
