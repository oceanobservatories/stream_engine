        ...
        with tempfile.NamedTemporaryFile() as tf:
            with netCDF4.Dataset(tf.name, 'w', format='NETCDF4') as ncfile:
                ...
                for chunk in self.generator.chunks(r):
                    ...
                    if deployment not in deployment_initialized:
                        index = 0
                        # create the time dimension in the root group
                        ncfile.createDimension('time_' + deployment, None)
                    else:
                        index = index = len(variables['7_' + deployment])

                    # create the netcdf variables and any extra dimensions
                    for param_id in chunk:
                        v_key = '%s_%s' % (param_id, deployment)
                        data = chunk[param_id]['data']

                        if deployment not in deployment_initialized:
                            source = chunk[param_id]['source']
                            g_key = '%s_%s' % (source, deployment)

                            if param_id == 7:
                                group = ncfile
                            else:
                                if g_key not in groups:
                                    groups[g_key] = ncfile.createGroup(g_key)
                                group = groups[g_key]
                            ...
                            dims = ['time_' + deployment]
                            if len(data.shape) > 1:
                                for index, dimension in enumerate(data.shape[1:]):
                                    name = '%s_dim_%d_%s' % (param_name, index, deployment)
                                    group.createDimension(name, dimension)
                                    dims.append(name)

                            try:
                                variables[v_key] = group.createVariable(param_name + '_' + deployment,
                                                                            data.dtype,
                                                                            dims,
                                                                            zlib=True)
                            except TypeError:
                    ...
                    deployment_initialized.add(deployment)
