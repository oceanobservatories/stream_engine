...
from util.advlogging import QueryInfo, CalculatedParameter, QueryData, QueryParameter, ParameterReport
...
@log_timing(log)
def get_netcdf(streams, start, stop, coefficients, qc_parameters, uflags=None, limit=None,
               include_provenance=False, include_annotations=False, strict_range=False, location_information={},
...
def calculate_derived_product(param, coeffs, pd_data, primary_key, provenance_metadata, stream_request, deployment,
    ...
    try:
        ...
        advLogFlagName = 'advancedStreamEngineLogging'
        advuName = 'userName'
        if stream_request.uflags is not None and advLogFlagName in stream_request.uflags:
            if stream_request.uflags[advLogFlagName] is True and advuName in stream_request.uflags:
                user = stream_request.uflags[advuName]
                report = ParameterReport()
                logname = primary_key.as_three_part_refdes() + "-" + stream_request.request_id
                report.setUserInfo(user, logname)
                report.setCalculatedParameter(param.id, param.name, param.parameter_function.function)
                for key, value in args.iteritems():
                    report.addParameterArgument(param.id, key, value.tolist())
                report.write()
...
class StreamRequest(object):
    ...
    def __init__(self, stream_keys, parameters, coefficients, time_range, uflags=None, qc_parameters={}, needs_only=False,
