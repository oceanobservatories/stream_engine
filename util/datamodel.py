...
def _group_by_stream_key(ds, pd_data, stream_key, location_information, deployment):
    ...
    if 'location_name' in location_vals:
        ds.attrs['location_name'] = location_vals['location_name']
    ...
    if 'lon' not in ds.variables:
        ...
        if lon is not None:
            ...
            ds.update({'lon' : v})
            ds.attrs['geospatial_lon_min']  = lat
            ds.attrs['geospatial_lon_max']  = lat
        else:
    ...
    depth_units = location_vals.get('depth_units', app.config["Z_DEFAULT_UNITS"])
