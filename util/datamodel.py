...
def to_xray_dataset(cols, data, stream_key, request_uuid, san=False, keep_exclusions=False):
    ...
    for deployment, dataframe_group in dataframe.groupby('deployment'):
        ...
        for column in dataframe.columns:
            ...
            if param:
                encoding = _get_value_encoding(param)
                ...
                fill_val = np.array(fill_val).astype(encoding)
                is_array = False
                param_dims = []
            ...
            # Don't set fill value for these so that the end result is an identical
            # NetCDF as that generated by the python 2 version of stream_engine
            if column in app.config['NO_FILL_VALUE']:
                fill_val = None

            # Override the fill value supplied by preload if necessary
            # Don't assign _FillValue if None otherwise this error will occur:
            # TypeError: Invalid value for attr '_FillValue': None.
...
def _replace_values(data_slice, value_encoding, fill_value, is_array, name):
    ...
    if data_slice.dtype == 'object' and not is_array:
        ...
        if np.any(nones):
            ...
            elif name not in app.config['NO_FILL_VALUE']:
                raise StreamEngineException('Fill value not found for param: %s, data type: %s' % (name, value_encoding))
...
def _get_value_encoding(param):
    return param.value_encoding if not param.value_encoding == 'string' else 'S'

def _get_fill_value(param):
    encoding = _get_value_encoding(param)
    try:
        return np.array(param.fill_value).astype(encoding)
    except (ValueError, TypeError) as e:
        log.error('Invalid fill value specified for parameter: %r (%r)', param, e)
        return FILL_VALUES.get(encoding)
