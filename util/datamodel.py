...
def _group_by_stream_key(ds, pd_data, stream_key, location_information, deployment, provenance_metadata):
    ...
    for param_id in pd_data:
        ...
        if param_name in ['lat', 'lon', 'loc_pressure']:
            array_attrs = {}
        else:
            array_attrs = {'coordinates' : 'time lat lon loc_pressure'}

        
        if param_name in ['loc_pressure']:
            array_attrs = {'comments' : 'Pressure from co-located CTD'}
        ...
        if param:
            ...
            if param_name == 'loc_pressure':
                array_attrs['long_name'] = "sea_water_pressure"
...
def fix_lat_lon_depth(ds, stream_key, deployment, location_information):
    ...
    if 'loc_pressure' not in ds.variables:
        depth = location_vals.get('loc_pressure')
        if depth is None:
        ...
        ds['loc_pressure'] = ('obs', deptharr, attrs)
