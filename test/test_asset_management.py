...
class AssetManagementTest(unittest.TestCase):
    def setUp(self):
        self.amhost = None  # 'ooiufs01.ooi.rutgers.edu'
        self.call_args = ('CE04OSPS', 'SF01B', '2A-CTDPFA107')
    ...
    def test_get(self):
        ...
        for subsite, node, sensor in targets:
            futures.append((subsite, node, sensor, am.get_events_async(subsite, node, sensor)))

        for subsite, node, sensor, f in futures:
            events = f.result()
    ...
    def test_get_calibrations(self):
        ...
        with requests_mock.Mocker() as m:
            m.get(am.get_events_url(*self.call_args), text=events_response)
            return am.get_events(*self.call_args).cals

    def test_get_calibrations_async(self):
        am = self.test_create_am()
        with requests_mock.Mocker() as m:
            m.get(am.get_events_url(*self.call_args), text=events_response)
            return am.get_events_async(*self.call_args).result().cals

    def test_get_events_async(self):
        am = self.test_create_am()
        with requests_mock.Mocker() as m:
            m.get(am.get_events_url(*self.call_args), text=events_response)
            future = am.get_events_async(*self.call_args)
    ...
    def test_expected_calibrations(self):
        ...
        # check name / deployment first
        self.assertEqual({(dep, name) for name in cals for dep in cals[name]}, {(e[0], e[1]) for e in expected})
        # check values
        # create a list of retrieved values in the same format as the expected
        found = [(dep, name, cals[name][dep].value) for name in cals for dep in cals[name]]
        found_vals = np.array([cal[2] for cal in sorted(found)])
        expected_vals = np.array([e[2] for e in sorted(expected)])

        np.testing.assert_almost_equal(found_vals, expected_vals, decimal=5)
