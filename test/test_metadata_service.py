...
class MockMetadataServiceAPI(object):
    ...
    def test_add_stream_metadata_record(self, subsite, node, sensor, method, stream, first, last, count):
        new_record = util.metadata_service.MetadataServiceAPI.build_stream_metadata_record(subsite, node, sensor, method, stream, first, last, count)
    ...
    def test_add_partition_metadata_record(self, subsite, node, sensor, method, stream, bin, store, first, last, count):
        new_record = util.metadata_service.MetadataServiceAPI.build_partition_metadata_record(subsite, node, sensor, method, stream, bin, store, first, last, count)
    ...
    def get_stream_metadata_record(self, subsite, node, sensor, method, stream):
        ...
        for rec in self.stream_metadata_records:
            if rec['referenceDesignator']['subsite'] == subsite and rec['referenceDesignator']['node'] == node and rec['referenceDesignator']['sensor'] == sensor \
               and rec['method'] == method and rec['stream'] == stream:
    ...
    def get_partition_metadata_records(self, subsite, node, sensor, method=None, stream=None):
        if method and stream:
            return [copy.deepcopy(rec) for rec in self.partition_metadata_records if rec['referenceDesignator']['subsite'] == subsite and rec['referenceDesignator']['node'] == node
                    and rec['referenceDesignator']['sensor'] == sensor and rec['method'] == method and rec['stream'] == stream]
        else:
            return [copy.deepcopy(rec) for rec in self.partition_metadata_records if rec['referenceDesignator']['subsite'] == subsite and rec['referenceDesignator']['node'] == node
                    and rec['referenceDesignator']['sensor'] == sensor]
...
class MetadataServiceTest(unittest.TestCase):
                ...
                for sensor_suffix in range(2):
                    for method_suffix in range(2):
                        for stream_suffix in range(2):
                            self.__stream_test_setup(1.1, 2.2, 3, subsite_suffix, node_suffix, sensor_suffix, method_suffix, stream_suffix)
        ########
        # Test #
        ########
    ...
    def test_get_location_metadata_by_store(self):
        ...
        sk = self.__partition_test_setup(bin_min - 1, util.metadata_service.CASS_LOCATION_NAME, 1.1, 6.6, 100)
        # Filtered - bad bin (too big), right store
        self.__partition_test_setup(bin_max + 1, util.metadata_service.CASS_LOCATION_NAME, 2.2, 5.5, 200)
        # Filtered - good bin, wrong store
        self.__partition_test_setup(bin_min + 1, util.metadata_service.SAN_LOCATION_NAME, 3.3, 4.4, 300)

        # 3 good ones
        self.__partition_test_setup(bin_min + 2, util.metadata_service.CASS_LOCATION_NAME, 4.4, 3.3, 11)
        self.__partition_test_setup(bin_min + 3, util.metadata_service.CASS_LOCATION_NAME, 5.5, 2.2, 12)
        self.__partition_test_setup(bin_min + 4, util.metadata_service.CASS_LOCATION_NAME, 6.6, 1.1, 13)
        ########
        # Test #
        ########
        actual_result = util.metadata_service.get_location_metadata_by_store(sk, tr, util.metadata_service.CASS_LOCATION_NAME)
    ...
    def test_get_first_before_metadata(self):
        ...
        sk = self.__partition_test_setup(bin_max + 1, util.metadata_service.CASS_LOCATION_NAME, bin_max, 1.1, 100)
        # Filtered - bad bin (5th - sorted out), good first
        self.__partition_test_setup(bin_max - 3, util.metadata_service.CASS_LOCATION_NAME, bin_max, 2.2, 101)
        # Filtered - good bin, bad first (too big)
        self.__partition_test_setup(bin_max, util.metadata_service.CASS_LOCATION_NAME, bin_max + 1, 3.3, 102)
        # Filtered - bad bin (3rd - ignored), good first
        self.__partition_test_setup(bin_max - 2, util.metadata_service.CASS_LOCATION_NAME, bin_max - 1, 4.4, 103)

        # Override config setting
        engine.app.config['PREFERRED_DATA_LOCATION'] = util.metadata_service.CASS_LOCATION_NAME

        # 1 good one
        self.__partition_test_setup(bin_max - 1, util.metadata_service.CASS_LOCATION_NAME, bin_max - 2, 5.5, 104)

        # Filtered - good bin, good first, wrong store
        self.__partition_test_setup(bin_max - 1, util.metadata_service.SAN_LOCATION_NAME, bin_max - 3, 6.6, 104)
        ########
        # Test #
        ########
        actual_result = util.metadata_service.get_first_before_metadata(sk, bin_max)
        self.assertIn(util.metadata_service.CASS_LOCATION_NAME, actual_result)
        self.assertEqual(actual_result[util.metadata_service.CASS_LOCATION_NAME].total, 104)
        self.assertEqual(actual_result[util.metadata_service.CASS_LOCATION_NAME].start_time, bin_max - 2)
        self.assertEqual(actual_result[util.metadata_service.CASS_LOCATION_NAME].end_time, 5.5)
        self.assertItemsEqual(actual_result[util.metadata_service.CASS_LOCATION_NAME].bin_list, [bin_max - 1])
        self.assertEqual(actual_result[util.metadata_service.CASS_LOCATION_NAME].bin_information, { bin_max - 1 : (104, bin_max - 2, 5.5) })
    ...
    def test_get_location_metadata(self):
        ...
        sk = self.__partition_test_setup(bin_min - 1, util.metadata_service.CASS_LOCATION_NAME, tr.stop, tr.start, 100)
        # Filtered - bad bin (too big), good first, good last
        self.__partition_test_setup(bin_max + 1, util.metadata_service.CASS_LOCATION_NAME, tr.stop, tr.start, 200)
        # Filtered - good bin, bad first (too big), good last
        self.__partition_test_setup(bin_min + 1, util.metadata_service.SAN_LOCATION_NAME, tr.stop + 1, tr.start, 300)
        # Filtered - good bin, good first, bad last (too small)
        self.__partition_test_setup(bin_min + 2, util.metadata_service.SAN_LOCATION_NAME, tr.stop, tr.start - 1, 400)

        # 4 good ones
        self.__partition_test_setup(bin_min + 3, util.metadata_service.CASS_LOCATION_NAME, tr.stop - 1, tr.start + 1, 11)
        self.__partition_test_setup(bin_min + 4, util.metadata_service.CASS_LOCATION_NAME, tr.stop - 2, tr.start + 2, 12)
        self.__partition_test_setup(bin_min + 5, util.metadata_service.SAN_LOCATION_NAME, tr.stop - 3, tr.start + 3, 13)
        self.__partition_test_setup(bin_min + 6, util.metadata_service.SAN_LOCATION_NAME, tr.stop - 4, tr.start + 4, 14)

        # Override config setting
        engine.app.config['PREFERRED_DATA_LOCATION'] = util.metadata_service.CASS_LOCATION_NAME

        # Filtered - In both CASS and SAN (same bin, same count) - without warning
        self.__partition_test_setup(bin_min + 3, util.metadata_service.SAN_LOCATION_NAME, tr.stop - 1, tr.start + 1, 11)
        # Filtered - In both CASS and SAN (same bin, different count) - with warning
        self.__partition_test_setup(bin_min + 4, util.metadata_service.SAN_LOCATION_NAME, tr.stop - 2, tr.start + 2, 10)
        ########
        # Test #
        ########
