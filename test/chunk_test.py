...
def test_average():
    cg = Test_Chunk_Generator()
    avg_chunk = { 7: {'source': 'foo', 'data': numpy.array([3,5,9,13])},
                  8: {'source': 'foo', 'data': numpy.array([2,3,4,7])},
                  9: {'source': 'foo', 'data': numpy.array([[1,2],[1,2],[1,2],[1,2]])},
                 10: {'source': 'foo', 'data': numpy.array(['str2','str3','str1','str1'])}}
    ...
    nan_chunk = chunks.copy(avg_chunk)
    nan_chunk[7]['data'] = numpy.array([1,2,3,4])
    for key in nan_chunk:
        if key != 7:
            nan_chunk[key]['data'][:] = numpy.NAN

    joined_chunk = chunks.join(*cg.chunks_list)
    chunk_result = chunks.average(chunks.copy(joined_chunk), [3,5,9,13])
...
def test_interpolate():
    cg = Test_Chunk_Generator()
    int_chunk = { 7: {'source': 'foo', 'data': numpy.array([3,5,9,13])},
                  8: {'source': 'foo', 'data': numpy.array([1.5,2.25,4,7])},
                  9: {'source': 'foo', 'data': numpy.array([[1,2],[1,2],[1,2],[1,2]])},
                 10: {'source': 'foo', 'data': numpy.array(['str1','str2','str1','str1'])}}
    joined_chunk = chunks.join(*cg.chunks_list)
    chunk_result = chunks.interpolate(joined_chunk, [3,5,9,13])
...
def test_average_generator():
    ...
    for chunk in ag.chunks(Test_StreamRequest([3,5,9,13])):
        cmp_chunk, avg_chunk = chunks.split(avg_chunk,1)
        assert chunks.is_equal(chunk, cmp_chunk)
...
def test_interpolation_generator():
    ...
    idx = 2
    for chunk in ag.chunks(Test_StreamRequest([3,5,9,13])):
        cmp_chunk, int_chunk = chunks.split(int_chunk,idx)
        idx = 1
        assert chunks.is_equal(chunk, cmp_chunk)