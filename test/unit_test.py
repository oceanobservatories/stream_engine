...
class StreamUnitTest(unittest.TestCase, StreamUnitTestMixin):
    ...
    def test_data_stream_with_limit(self):
        ...
        # first, verify we get 2n data points from chunk generator
        particles = json.loads(''.join(list(Particle_Generator(Chunk_Generator()).chunks(sr))))
        self.assertEqual(len(particles), 2*number_points)

        # second, verify the times bracket our input times
        retrieved_times = [p['time'] for p in particles]
        for i, t in enumerate(times):
            self.assertLessEqual(retrieved_times[i*2], t)
            self.assertGreaterEqual(retrieved_times[i*2+1], t)

        # finally, verify our results are interpolated to our expected time frame
        particles = json.loads(''.join(list(Particle_Generator(Interpolation_Generator(Chunk_Generator())).chunks(sr))))
        numpy.testing.assert_array_equal(times, [p['time'] for p in particles])
