...
class ReleaseNotesTest(unittest.TestCase):
    ...
    def test_parse_current_release_notes(self):
        ...
        component = actual_release.get_component()
        self.assertIsNotNone(component)

    def test_get_component_name(self):
        name = self.release.get_component_name()
        self.assertEqual(name, "Stream Engine")

    def test_get_latest_version(self):
        version_number = self.release.get_latest_version()
        self.assertEqual(version_number, "1.10.0")

    def test_get_latest_date(self):
        date = self.release.get_latest_date()
        self.assertEqual(date, "2019-07-19")

    def test_get_latest_descriptor(self):
        descriptor = self.release.get_latest_descriptor()
        self.assertEqual(descriptor, "Development Release")

    def test_get_component(self):
        component = self.release.get_component()
        self.assertIsNotNone(component)

    def test_get_dependencies(self):
        dependencies = self.release.get_dependencies()
    ...
    def test_unknown_version(self, mock_parse):
        ...
        version = self.release.get_latest_version()
