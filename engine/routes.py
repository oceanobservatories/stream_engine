...
def netcdf():
    ...
    """
    # TODO - make this return the netCDF directly, currently same as the async endpoint
    input_data = request.get_json()
    return Response(util.calc.get_netcdf(input_data, request.url), mimetype='application/json')
...
def _delimited_fs(delimiter):
    ...
    base_path = os.path.join(app.config['ASYNC_DOWNLOAD_BASE_DIR'], input_data.get('directory', 'unknown'))
    try:
        json_str = util.calc.get_csv_fs(input_data, request.url, base_path, delimiter='\t')
    except Exception as e:
        json_str = output_async_error(input_data, e)
