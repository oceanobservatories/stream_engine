...
def time_prefex_filename(ntp_start, ntp_stop, suffix):
...
def netcdf_save_to_filesystem():
    ...
    try:
        ...
        json_efile = time_prefex_filename(input_data.get('start'), input_data.get('stop'), "falure.json")
        json_str = output_async_error(input_data, e, filename=json_efile)

    status_filename = time_prefex_filename(input_data.get('start'), input_data.get('stop'), "status.txt")
...
def particles_save_to_filesystem():
    ...
    try:
        ...
        filename = time_prefex_filename(input_data.get('start'), input_data.get('stop'), "failure.json")
        file_path = os.path.join(base_path, filename)
